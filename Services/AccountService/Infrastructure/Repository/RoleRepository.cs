...
{
    public class RoleRepository : RepositoryBase<EfDbContext, Domain.Entities.Role, Role>, Domain.Repository.IRoleRepository
    {
        private readonly EfDbContext context;
        public RoleRepository(EfDbContext context) : base(context) { this.context = context; }
        public override void Add(Domain.Entities.Role t)
        {
            var role = t.CopyTo<Domain.Entities.Role, Role>();
            context.Role.Add(role);
            handleRolePermission(role);
        }
        public override void Update(Domain.Entities.Role t)
        {
            var role = t.CopyTo<Domain.Entities.Role, Role>();
            context.Role.Update(role);
            handleRolePermission(role);
        }
        public override void Delete(Domain.Entities.Role t)
        {
            var role = t.CopyTo<Domain.Entities.Role, Role>();
