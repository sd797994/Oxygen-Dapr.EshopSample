...
export default {
  ...
  mounted() {
    this.checkRolebasedInit()
  },
  methods: {
    checkRolebasedInit() {
      this.$store.dispatch('user/checkRolebasedInit').then((resp) => {
        this.initusername = resp.data.loginName
        this.inituserpwd = resp.data.password
      }).catch((e) => {
        this.loading = false
      })
    },
